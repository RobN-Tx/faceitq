...
class FaceitMatch:
    """its big, its chunk and it stores the data for an on going faceit match"""
    ...
    def finish_match(self):
        ...
        try:
            ...
            self.score.ot_processor(
                self.score.match_score.ct_start, self.score.match_score.t_start
            )
    ...
    def build_embedded_message(self):
        ...
        embeded_message = discord.Embed(
            title=title_text,
            description="# " + map_name + "\n On " + self.game_data.hub,
            color=0xFF5500,
        )

        embeded_message.set_thumbnail(url=self.game_data.map_img)
        embeded_message.add_field(
            name="`" + self.ct_start.team_name + "`",
            value=self.ct_start.team_listing,
            inline=True,
        )

        score_detail = (
            self.score.get_final_scores_string()
            if self.finished
            else self.score.get_ot_scores_string()
        )

        embeded_message.add_field(
            name=self.score.get_match_score_string(), value=score_detail, inline=True
        )
        embeded_message.add_field(
            name="`" + self.t_start.team_name + "`",
            value=self.t_start.team_listing,
            inline=True,
        )
        embeded_message.add_field(
            name=" ",
            value="[Summary](https://www.faceit.com/en/cs2/room/" + self.match_id + ")",
        )

        if self.demoed:
            embeded_message.add_field(name=" ", value="--", inline=True)
...
class MatchScore:
    ...
    def update_scores(self, summary_results_dict):
        ...
        if "factions" in summary_results_dict.keys():
            ...
            self.match_score.ct_start = summary_results_dict["factions"]["faction1"][
                "score"
            ]
            self.match_score.t_start = summary_results_dict["factions"]["faction2"][
                "score"
            ]

            self.ot_processor(
                int(self.match_score.ct_start), int(self.match_score.t_start)
            )
